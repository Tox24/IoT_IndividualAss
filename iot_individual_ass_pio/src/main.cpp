...
void vFFTTask(void *args) {
    ...
    for(;;) {
        ...
        ESP_LOGI(TAG, "Setting frequency to: %.2f Hz\n", new_sample_freq);
...
static void sync_time(void) {
    ...
    ESP_LOGI(TAG, "SUCCESS: %s", asctime(&timeinfo));
}
...
static void event_handler(void* arg, esp_event_base_t event_base, int32_t event_id, void* event_data) {
    if (event_base == WIFI_EVENT && event_id == WIFI_EVENT_STA_START) {
        ...
        if (s_retry_num < 10) { 
            esp_wifi_connect();
            s_retry_num++;
            //ESP_LOGW(TAG, "Retrying connection to Wi-Fi... (%d/10)", s_retry_num);
        } else {
            xEventGroupSetBits(s_wifi_event_group, WIFI_FAIL_BIT);
        }
    } else if (event_base == IP_EVENT && event_id == IP_EVENT_STA_GOT_IP) {
        ip_event_got_ip_t* event = (ip_event_got_ip_t*) event_data;
        ESP_LOGI(TAG, "SUCCESS - IP Address obtained: " IPSTR, IP2STR(&event->ip_info.ip));
        s_retry_num = 0;
...
static void wifi_init_sta(void) {
    ...
    if (bits & WIFI_CONNECTED_BIT) {
        ...
        //ESP_LOGE(TAG, "Impossible to connect to Wi-Fi. Stopping...");
        vTaskSuspend(NULL);
    }
}
